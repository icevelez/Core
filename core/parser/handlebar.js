import { add_block_to_cache, create_render_code, make_id } from "../runtime.js";

export function parser(source) {
    const block_pattern = /{{#(await|if|each)(.*?)}}|{{\/(await|if|each)}}/gs, stack = [], blocks = [];
    let match;
...
const parse = {
    if: function (block) {
        ...
        while ((m = RE.else.exec(firstBody))) {
            if (m.index > lastIndex) {
                exprs.push(lastCond);
                fns.push(create_render_code(firstBody.slice(lastIndex, m.index)))
            }
            if (m[0].startsWith("{{:else if")) {
                ...
                exprs.push("true");
                fns.push(create_render_code(firstBody.slice(m.index + m[0].length)))
        ...
        if (lastIndex < firstBody.length) {
            exprs.push(lastCond);
            fns.push(create_render_code(firstBody.slice(lastIndex)))
        }
    ...
    each: function (block) {
        ...
        return {
            expr: expr.trim(),
            fn: parts[0] ? create_render_code(parts[0]) : undefined,
            else_fn: parts[1] ? create_render_code(parts[1]) : undefined,
            key: trimmedVar,
    ...
    await: function (block) {
        ...
        return {
            ...
            pending_fn: pending ? create_render_code(pending) : undefined,
            then_fn: thenMatch && thenMatch[2] ? create_render_code(thenMatch[2]) : undefined,
            then_key: thenMatch && thenMatch[1] || undefined,
            catch_fn: catchMatch && catchMatch[2] ? create_render_code(catchMatch[2]) : undefined,
