 ...
 * 4. Error getter
 */
export function create_managed_value(initial_value, actions) {
    const action_keys = Object.keys(actions);
    const [value, setValue] = signal(initial_value);
    ...
    for (const key of action_keys) defined_actions[key] = function (...args) {
        const new_value = actions[key](value(), ...args);
        if (new_value instanceof Promise) {
            setPending(true);
            return new_value.then((value) => { setValue(value); setError(null); }).catch(error => setError(error)).finally(() => setPending(false));
        }
        setValue(new_value);
    }
